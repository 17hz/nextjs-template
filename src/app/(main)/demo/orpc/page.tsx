  ...
  return (
    <div className="min-h-screen bg-black p-8 text-white">
      <div className="mx-auto max-w-2xl">
        <h1 className="mb-8 font-bold text-4xl tracking-tighter">oRPC Demo</h1>

        <form onSubmit={handleSubmit} className="mb-8 space-y-4">
          <div>
            <label className="mb-2 block font-mono text-gray-400 text-sm" htmlFor="">
              Planet Name
            </label>
            <input
              type="text"
        ...
        <div className="border-gray-800 border-t pt-8">
          <h2 className="mb-4 font-mono text-gray-400 text-xl">Planets</h2>

          {planetsQuery.isLoading && <p className="font-mono text-gray-500">Loading...</p>}

          {planetsQuery.error && (
            <p className="font-mono text-red-500">Error: {planetsQuery.error.message}</p>
          )}

          {planetsQuery.data && (
            <ul className="space-y-3">
              {planetsQuery.data.map((planet) => (
                <li key={planet.id} className="border border-gray-800 p-4">
                  <div className="font-bold">{planet.name}</div>
                  {planet.description && (
                    <div className="mt-1 text-gray-400 text-sm">{planet.description}</div>
                  )}
                </li>
              ))}
            </ul>
